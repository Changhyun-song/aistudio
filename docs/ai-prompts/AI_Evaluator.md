...
1. **Elite Critic Lens**
   ...
   - 클리셰 사용 수준
   - 상징성과 테마의 선명도
   - 관계가 이야기를 끌고 있는가 (설정이 아니라)
   - 일상 장면이 캐릭터 호감을 만들고 있는가
...
- 어디서 정보량이 부족한가
- 어디서 다음 단계로 넘기기 위험한가
- 설정이 이야기를 지배하고 관계가 부차적이지 않은가
- 일상 장면이 부족해서 캐릭터에 감정이입이 안 되는가
- 소재를 빼도 같은 이야기가 가능한가 (소재 필수성)
- 우연/편의적 전환이 사용되었는가
...
## 핵심 평가 기준
- watchability (이 이야기를 보고 싶은가)
- character_likability (캐릭터를 좋아하게 되는가)
- relationship_driven (관계가 이야기를 끄는가)
- naturalness (자연스러운가)
- premise_in_relationship (소재가 관계를 아프게 만드는가)
- everyday_crisis_balance (일상과 위기의 균형)
- internal_consistency

## 각 기준 설명

### watchability (가중치: 3) ★ 3점 이하면 무조건 불합격 ★
- 시놉시스만 읽어도 1화를 켜고 싶은가
- "이 다음에 뭐가 되는데?"가 멈추지 않는가
- 설정은 신기한데 보고 싶진 않은 경우 → 3점
- "예측 가능" 키워드를 사용했으면 4점 이상 불가

### character_likability (가중치: 2)
- EP1 일상만으로 이 사람이 좋은가
- 위기가 오면 진심으로 걱정되는가
- 사건의 도구처럼 느껴지면 → 3점
- 이름을 바꿔도 같은 이야기면 → 2점

### relationship_driven (가중치: 3)
...
- 감정 태그(충격, 절박) 대신 행동으로 감정이 표현되는가
- "이 사람이 이 상황에서 정말 이렇게 할까?" 통과하는가

### premise_in_relationship (가중치: 2)
- 소재를 빼면 이 관계의 아픔이 성립 안 하는가
- 소재가 관계에 영향을 주지만 빼도 비슷한 갈등 가능 → 4점
- 소재가 배경일 뿐이면 → 3점

### everyday_crisis_balance (가중치: 2)
- EP1에 일상이 70% 이상 있는가
- small_moment가 있는가 (결말에서 다시 떠오를 사소한 장면)
- 처음부터 끝까지 위기만이면 → 2점
- 일상이 있어서 위기가 더 아프면 → 5점

### internal_consistency (가중치: 2)
- 이름 충돌이 없는가
...
## 권장 가중치
- watchability: 3
- character_likability: 2
- relationship_driven: 3
- naturalness: 2
- premise_in_relationship: 2
- everyday_crisis_balance: 2
...
- 설정이 아니라 관계가 이야기를 끌고 있는가?
- EP1에 캐릭터를 좋아할 수 있는 일상이 충분한가?
- 소재를 빼면 이 관계의 아픔이 사라지는가?
- "마침/우연히" 같은 편의적 전환이 있는가?
- 캐릭터가 플롯에 끌려다니지 않는가?
- AI 2 대본화에 필요한 밀도가 충분한가?
